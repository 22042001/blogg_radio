...
@section('content')
    ...
    <form action="{{ route('conductores.update', $conductor->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nombre" id="nombre" class="form-control" value="{{ $conductor->nombre }}" required>
        </div>
        <div class="mb-3">
            ...
            <select name="programas_nombre_unico" id="programas_nombre_unico" class="form-control" required>
                @foreach ($programas as $programa)
                    <option value="{{ $programa->nombre_unico }}" {{ $conductor->programas_nombre_unico == $programa->nombre_unico ? 'selected' : '' }}>
